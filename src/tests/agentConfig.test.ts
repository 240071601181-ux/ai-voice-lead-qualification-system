...
describe('Agent configuration API', () => {
  ...
  it('persists an edited greeting (PATCH round-trip)', async () => {
    ...
    const reread = await request(app).get('/api/v1/agent/config');
  ...
  it('rejects unknown fields and empty patches', async () => {
    ...
    const empty = await request(app).patch('/api/v1/agent/config').send({});
    expect(empty.status).toBe(400);

    const badQuestions = await request(app)
      .patch('/api/v1/agent/config')
  ...
  it('pauses and resumes the agent with persistence', async () => {
    const paused = await request(app).post('/api/v1/agent/pause');
    ...
    const resumed = await request(app).post('/api/v1/agent/resume');
